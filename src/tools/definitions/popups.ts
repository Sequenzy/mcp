...
const POPUP_VISUAL_DESCRIPTION =
  'Media panel and urgency treatment: { "style": "none" | "accent" | "header" | "rail" | "image" | "countdown", "placement": "top" | "left" | "center" | "right", "imageUrl": "https://...", "imageAlt": "...", "countdownMinutes": 1-10080 }. Use style "image" with an imageUrl for a product panel, or "countdown" with countdownMinutes for a deadline. Merged into the current visual.';

const POPUP_BLOCKS_DESCRIPTION =
  'Full replacement for the popup\'s content blocks. Fetch the current blocks from get_popup, modify them, and send the complete array - the popup must keep exactly one required email field and one submit button. Blocks render in array order; every block needs a unique "id" and a "kind", and "sectionId" ("success" or "error") moves a content block into that screen. Input blocks are kind "form-field": { id, kind: "form-field", fieldType, name, label, placeholder, required, defaultValue, showLabel, width ("full" | "half"), mapsTo, options }. fieldType is one of text, email, phone, number, textarea, select, radio, checkbox, consent, hidden. mapsTo is one of email, firstName, lastName, phone, customAttribute and defaults to customAttribute, which stores the answer under the block\'s "name" as a subscriber custom attribute. select, radio, and checkbox fields require options: [{ value, label, id }] - label and id default to value when omitted. Other kinds: heading, text, image, button, divider, spacer, custom-html, feature-grid, testimonial, countdown, form-step, submit-button, success-screen, error-state. Validation errors name the offending property, for example "blocks[3].options[0].value".';
